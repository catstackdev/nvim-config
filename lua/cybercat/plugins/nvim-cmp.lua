return {
			...
			config = function()
				require("copilot").setup({
					...
					filetypes = {
						...
						svn = false,
						cvs = false,
						-- Shader files: copilot uses UTF-16 position encoding while
						-- glsl_analyzer / wgsl_analyzer use UTF-8. Having both
						-- attached causes `_range.lua:212: Index out of bounds`
						-- crashes on K (hover). Detach copilot from shaders.
						glsl = false,
						wgsl = false,
						frag = false,
						vert = false,
	...
	config = function()
		...
		-- Custom GLSL struct-field completion source. Mirrors the web
		-- editor's structFieldCompletions — pops field names on `<var>.`
		-- when <var> is declared as a known struct type (Material,
		-- BrickConfig, …). Data loaded from
		-- ~/.config/nvim/snippets/glsl-structs.json, regenerated by
		-- `pnpm snippets:install` in the glsl-workspace repo.
		cmp.register_source("glsl_struct", require("cybercat.cmp_glsl_struct").new())
